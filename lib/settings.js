...
const KEY_USE_BUILTIN_ALLOWLIST = "useBuiltinAllowlist";
...
// === 內建知名網站白名單（F5）開關 ===
// 開啟後：命中內建白名單（CrUX 台灣+全球榜經過濾打包,見 lib/allowlist.js）的
// 網域直接放行、不查 RDAP —— 同時降低誤判與隱私外洩。使用者黑名單永遠優先
// （evaluateDomain 的短路順序保證）。預設開;選項頁可關。

export function getUseBuiltinAllowlist() {
  return getBoolFlag(KEY_USE_BUILTIN_ALLOWLIST, true);
}

export function setUseBuiltinAllowlist(value) {
  return setBoolFlag(KEY_USE_BUILTIN_ALLOWLIST, value);
}
