...
export async function getCache(domain) {
  ...
  try {
    ...
    // ageDays 在寫入當下被凍結,但網域會持續變老。7 天 TTL 內若不重算,
    // 一個「抓取時 28 天、現已 35 天」的網域仍會被當成 < 30 天門檻而持續紅色告警,
    // 也會連帶卡住 young 鎖定守門。registrationDate 已存在 cache value 裡,重算零成本。
    if (entry.registrationDate) {
      const t = new Date(entry.registrationDate).getTime();
      if (!isNaN(t)) entry.ageDays = Math.floor((Date.now() - t) / 86400000);
    }
    return entry;
  } catch (err) {
