...
import { ALLOWLIST, ALLOWLIST_META } from "./allowlist.js";

const WHITELIST_KEY = "whitelist";
const BLACKLIST_KEY = "blacklist";

// === 內建知名網站白名單（F5,build-time 產生,見 scripts/build-allowlist.py）===
// CrUX 真實流量榜（台灣 top-5000 + 全球 top-1000）經 PSL PRIVATE / 排除表 /
// 165 涉詐名單過濾。命中即放行不查 RDAP;絕不覆蓋使用者黑名單（呼叫端順序保證）。
const ALLOWLIST_SET = new Set(ALLOWLIST);

/** 是否命中內建白名單。傳入值必須是 eTLD+1（與名單口徑一致）。 */
export const isWellKnown = (domain) => ALLOWLIST_SET.has(domain);

/** 內建白名單的版本資訊（給選項頁顯示用）。 */
export const getAllowlistMeta = () => ({ ...ALLOWLIST_META });
 ...
 *   - .tech / .shop / .store / .live / .support / .world / .fun
 ...
 * 以避免誤殺合法網站。
 *
 * 比對方式：對 RDAP 回傳的 registrar.name 小寫後比對。純 ASCII 清單詞用
 * 詞界比對（"namecheap" 命中「Namecheap, Inc.」，但 "ename" 不會命中「Onename」）；
 * 含 CJK 的清單詞（如「西维数码」）詞界概念不適用，維持子字串包含比對。
 *
...
/**
 * 判斷註冊商名稱是否命中高風險清單。
 * 純 ASCII 清單詞用詞界比對，含 CJK 清單詞用子字串包含比對（理由見迴圈內註解）。
 *
...
export function isHighRiskRegistrar(registrarName) {
  ...
  for (const needle of HIGH_RISK_REGISTRARS) {
    // 含非 ASCII（如中文「西维数码」）的 needle 詞界概念不適用,沿用子字串比對。
    // 純 ASCII 的 needle 改用詞界比對,避免過短字串（如 "ename"）命中無關名稱
    // （如 "Onename"）而把合法註冊商誤標為高風險 —— 在防詐工具裡誤殺即信任流失。
...
/**
 * 把使用者輸入化約成 registrable domain（eTLD+1），與 background 比對名單時的口徑一致。
 *
 * 比對時 background 一律用 extractRootDomain(url) 得到的 eTLD+1（如 "evil.com"），
 * 但使用者可能輸入子網域（"www.evil.com"）、含 scheme 的 URL（"https://evil.com/login"）
 * 或裸網域。若原樣存入，查詢時永遠比對不到 —— 黑名單會「靜默失效」給出假保護。
 * 這裡統一化約成 eTLD+1：先剝 / 補 scheme，再走與 background 相同的 PSL 解析。
 *
 * @param {string} input
 * @returns {string|null} eTLD+1；IP / 單段 host / 無法解析時回 null（呼叫端應拒絕並提示）
 */
function toRegistrableDomain(input) {
  if (!input || typeof input !== "string") return null;
  const trimmed = input.trim();
  if (!trimmed) return null;
  const urlStr = /^[a-z][a-z0-9+.-]*:\/\//i.test(trimmed) ? trimmed : `https://${normalize(trimmed)}`;
  return extractRootDomain(urlStr);
}

async function addTo(key, domain) {
  const d = toRegistrableDomain(domain);
  if (!d) return { ok: false, reason: "invalid domain" };
...
async function removeFrom(key, domain) {
  // 移除時寬鬆：normalize 原形與化約形（eTLD+1）兩個候選都試。先比原形，
  // 才能清掉舊版本可能存入的非 eTLD+1 項（如 "www.evil.com"——化約後反而比對不到）；
  // 再比化約形，讓子網域 / 完整網址輸入也能移除對應的 eTLD+1 項。
  const candidates = [...new Set([normalize(domain), toRegistrableDomain(domain)].filter(Boolean))];
  ...
  return { ok: true, notFound: true, list };
}
...
export const isWhitelisted = async (domain) => {
  const d = toRegistrableDomain(domain) || normalize(domain);
  if (!d) return false;
