 ...
 * inspectDomainName(domain)：判斷 registrable domain 的「主域名」是否奇怪
 *                        （過長 / 含連字號），供「偵測奇怪域名」功能使用。
 */
...
export function extractRootDomain(url) {
  ...
  // 去掉前後的點（FQDN 尾點;連續多點一併剝除 —— WHATWG URL 允許空 label,
  // 只剝一個點會把 "google.com.." 算成畸形的 "com.",與 build 管線口徑不一致）
  host = host.replace(/^\.+|\.+$/g, "");
...
// 「主域名過長」門檻：label 字數（不含 TLD / public suffix）超過此值即視為偏長。
// 規則為「超過 10 個字」→ 嚴格大於 10（11 字以上才命中）。
export const ODD_NAME_MAX_LABEL_LEN = 10;

/**
 * 判斷 registrable domain（eTLD+1）的「主域名」是否奇怪。
 *
 * 主域名 = registrable domain 去掉 public suffix 後最左邊那一個 label。
 * 因為 eTLD+1 依定義就是「public suffix + 再往左一個 label」,該 label 恆為
 * 字串中的第一段,故直接取 split(".")[0] 即為主域名（已自然排除 TLD,
 * 含台灣常見的 com.tw / org.tw 這類多段 suffix）。
 *
 * 命中規則（任一即奇怪）：
 *   - 主域名長度 > ODD_NAME_MAX_LABEL_LEN（10）          → reason "long"
 *   - 主域名含連字號 "-"                                  → reason "hyphen"
 *
 * IDN 例外：punycode（"xn--" 開頭）的 label 是 ASCII 編碼後的字串,
 * 長度被放大、且必然含 "-",照字面套規則會把所有國際化網域（含正常的
 * 中文網域）全部誤判為奇怪。故 "xn--" 開頭的主域名一律跳過,不視為奇怪。
 *
 * @param {string} registrableDomain eTLD+1,如 "example.com"
 * @returns {{ odd: boolean, label: string|null, reasons: string[] }}
 *          reasons ⊆ {"long","hyphen"}
 */
export function inspectDomainName(registrableDomain) {
  const empty = { odd: false, label: null, reasons: [] };
  if (!registrableDomain || typeof registrableDomain !== "string") return empty;

  const label = registrableDomain.split(".")[0];
  if (!label) return empty;

  // IDN / punycode：編碼後字串失真,直接跳過（見上方說明）。
  if (label.startsWith("xn--")) return { odd: false, label, reasons: [] };

  const reasons = [];
  if (label.length > ODD_NAME_MAX_LABEL_LEN) reasons.push("long");
  if (label.includes("-")) reasons.push("hyphen");

  return { odd: reasons.length > 0, label, reasons };
}
